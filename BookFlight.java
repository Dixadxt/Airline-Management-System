...
public  class BookFlight extends JFrame implements ActionListener{
    ...
    JButton bookflight, fetchButton, flight,payButton;
...
public  BookFlight() {
   ...
   payButton.setBounds(50, 580, 150, 25);
...
public void actionPerformed(ActionEvent ae){
    ...
    try {
     ...
     if(rs.next()){
        ...
        // Payment Simulation
        int result = JOptionPane.showConfirmDialog(null, "Do you want to proceed with payment?", "Payment Confirmation", JOptionPane.YES_NO_OPTION);
        if (result == JOptionPane.YES_OPTION) {
            isPaymentComplete = true; // Mark payment as complete
            JOptionPane.showMessageDialog(null, "Payment Successful!");
        } else {
            isPaymentComplete = false; // Reset payment status
        }

    } else if (ae.getSource() == bookflight) {
        // Booking Flight (only if payment is complete)
        if (!isPaymentComplete) {
            JOptionPane.showMessageDialog(null, "Please complete the payment before booking!");
            return;
